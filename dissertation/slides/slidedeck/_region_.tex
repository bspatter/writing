\message{ !name(part_bioeffects_defense.tex)}
\message{ !name(part_bioeffects_defense.tex) !offset(-2) }
6~\begin{frame}
  \begin{center}
    {\LARGE Part II: Diagnostic ultrasound bioeffects\\}
  \end{center}
%  \vspace*{0.5cm}
  \begin{minipage}{0.48\textwidth}
    \begin{figure}
      Contrast-enhanced ultrasound:\\Cavitation bioeffects\vspace*{4pt}\\
      \centering
    \begin{tikzpicture}
      \node[anchor=south west,inner sep=0] (image) at (0,0) {
        \includegraphics[height=0.26\textheight]{./figs/CEUS}
      };%
    ...
    \begin{tikzpicture}
      \node[anchor=south west,inner sep=0] (image) at (0,0) {
        \includegraphics[height=0.26\textheight]{./figs/Kidney_Bleed}\hfill
      };%
      \begin{scope}[x={(image.south east)},y={(image.north west)}]%
...
\message{ !name(part_bioeffects_defense.tex) !offset(-72) }
