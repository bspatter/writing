...
\begin{frame}\frametitle{}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}
      ...
      text         = black,] 
      (T)
      {\quad Tissue \vspace{0.6\textwidth}\\%
        \includegraphics[width=\textwidth]{./figs/ultrasound_example_transparent}
      };

      \node [box, right  = 0cm of T, fill = blue!20!white]
      (A)
      {\quad Air \\ \vspace{\textwidth} \quad };
    \end{tikzpicture}
  \end{figure}
\end{frame}
% 
...
\invisible<1>{
 ...
 \item Calculate approximate interface stresses and strains for clinically relevant ultrasound waves \vspace{0.25cm}%
 \item Compare calculated stresses and strains with expected alveolar failure thresholds  \vspace{0.25cm}%
 \end{enumerate}
} \vfill
\end{frame}
...
% 
% 
\begin{frame}\frametitle{The approximate viscous stress is calculated}
  ...
  \begin{figure}
    ...
    \begin{subfigure}[b]{0.3\textwidth}
      \begin{tikzpicture}%
        ...
        \begin{scope}[x={(image.south east)},y={(image.north west)}]%
          \node[font=\small,right] at (0.3,0.9) {$\tau_{xy}$ (Pa)};%
        \end{scope}%  
      \end{tikzpicture}%
